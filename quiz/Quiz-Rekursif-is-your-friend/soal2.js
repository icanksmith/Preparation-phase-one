...
function changeXRecursive(data, jenis) {
  ...
  const restChar = data.slice(1);
  let processedChar = firstChar;
  if (jenis === 'ganjil'){
    if (Number(processedChar) % 2 === 0){
        processedChar = 'x';
    }
  } else if (jenis === 'genap') {
    if (Number(processedChar) % 2 === 1){
        processedChar = 'x';
    }
  }
  return processedChar + changeXRecursive(restChar, jenis);
}

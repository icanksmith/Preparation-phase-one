...
function travelingIndonesia(arr, emoney) {
  ...
  for (let i = 0; i < arr.length; i++){
    ...
    for (let k = 0; k < destination.length; k++){
      if (destination[k] === data.departureCity) indexAsal = k;
      else if (destination[k] === data.destinationCity) indexTujuan = k;
    }
    const hops = Math.abs(indexTujuan - indexAsal);
    const baseCost = hops * transportPrice[data.transport];
    data.totalCost = baseCost - (baseCost * paymentDiscount[emoney]);
    ...
    for (let l = 0; l < result.length; l++){
      if (data.totalCost > result[l].totalCost){
        result.push(data);
        for (let m = result.length - 1; m > l; m--){
          result[m] = result[m - 1];
        }
        result[l] = data;
        inserted = true;

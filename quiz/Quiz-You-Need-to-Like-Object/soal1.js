...
function ganjilGenapAsianGames(date, data) {
    ...
    for (const mobil of data) {
        if (mobil.type === 'Mobil'){
            const platNum = mobil.plat.replace(/[^0-9]/g, '');
            const lastPlatNum = Number(platNum.charAt(platNum.length - 1));
            
            if (date % 2 === 0 && lastPlatNum % 2 === 1) counter++;
            else if (date % 2 === 1 && lastPlatNum % 2 === 0) counter++;

            // baru nyadar bisa begini 
            // if ((date % 2) !== (lastPlatNum % 2)) counter++;
            // lebih simple tpi takut gabisa baca ntar :v
        }
    }
    return counter;
}

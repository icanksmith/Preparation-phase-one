...
function economyChangeSummary(tradeActivity) {
  ...
  for (let i = 0; i < tradeActivity.length; i++){
    for (let j = 0; j < tradeActivity[i].length; j++){
      const data = tradeActivity[i][j];
      const name = data.replace(/[^a-zA-Z\s]/g, '').trim();
      const operator = data.replace(/[^+-]/g, '').trim();
      const digit = Number(data.replace(/[^0-9]/g, '').trim()) / 100;
      let owner = '';
      
      if (name === 'Jeff Bezos') owner = 'Amazon';
      else if (name === 'Larry Page') owner = 'Google';
      else if (name === 'Jack Ma') owner = 'Alibaba';

      let newDeposit = 0;
      if (name === 'Jeff Bezos') newDeposit = duitJeff;
      else if (name === 'Larry Page') newDeposit = duitLarry;
      else if (name === 'Jack Ma') newDeposit = duitJack;
      
      if (operator === '+'){
        newDeposit = newDeposit + (newDeposit * digit);
      } else {
        newDeposit = newDeposit - (newDeposit * digit);
      }
      ...
      if (name === 'Jeff Bezos') duitJeff = newDeposit;
      else if (name === 'Larry Page') duitLarry = newDeposit;
      else if (name === 'Jack Ma') duitJack = newDeposit;
    }
  }
  return result;
}
